  ...
  <div v-if="loginModalStore.abierto" ref="modal" role="dialog" aria-modal="true" aria-labelledby="login-modal-titulo" aria-describedby="login-modal-descripcion" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/55 px-3 py-4 backdrop-blur-[2px] sm:px-4 sm:py-6" @click.self="loginModalStore.cerrar()">
    <form @submit.prevent="onSubmit" class="relative max-h-[calc(100dvh-2rem)] w-full max-w-sm space-y-4 overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl sm:max-h-[calc(100dvh-3rem)] sm:p-8">
      ...
      <div class="flex justify-center px-8">
        <img src="@assets/logo.webp" alt="Quiniela Futbol Americano" class="h-auto w-full max-w-[15rem] object-contain" />
      </div>
      ...
      <button type="submit" :disabled="cargando" class="min-h-11 w-full rounded-xl bg-quiniela-rojo px-4 py-2.5 font-semibold text-white transition hover:bg-quiniela-rojoOscuro focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-quiniela-azul focus-visible:ring-offset-2 disabled:cursor-wait disabled:opacity-70">
        {{ cargando ? 'Entrando...' : 'Iniciar sesión' }}
      </button>
      <div class="flex flex-col items-center gap-1 text-center text-sm sm:flex-row sm:justify-center sm:gap-4">
        <router-link :to="{ name: 'registro' }" @click="loginModalStore.cerrar()" class="rounded-md px-2 py-1.5 font-semibold text-quiniela-azul hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-quiniela-azul">Crear cuenta</router-link>
        <router-link :to="{ name: 'recuperar-password' }" @click="loginModalStore.cerrar()" class="rounded-md px-2 py-1.5 text-quiniela-azul hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-quiniela-azul">Olvidé mi contraseña</router-link>
      </div>
    </form>
  </div>
</template>
