...
<template>
  <label class="relative block text-sm font-semibold">
    {{ label }}
    <div class="relative mt-1"><img v-if="modelValue?.logo" :src="modelValue.logo" alt="" class="absolute left-3 top-1/2 h-7 w-7 -translate-y-1/2 object-contain" /><input v-model="query" role="combobox" aria-autocomplete="list" :aria-expanded="Boolean(options.length)" autocomplete="off" class="form-control pr-10" :class="modelValue?.logo ? 'pl-12' : ''" :placeholder="loading ? 'Buscando…' : 'Busca un equipo NFL'" /><span v-if="loading" class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400" aria-hidden="true">•••</span></div>
    <ul v-if="options.length" role="listbox" class="absolute z-30 mt-1 max-h-56 w-full overflow-auto rounded-xl border border-gray-200 bg-white p-1 shadow-xl">
      <li v-for="team in options" :key="team.id" role="option"><button type="button" @click="select(team)" class="flex min-h-11 w-full items-center gap-3 rounded-lg p-2 text-left hover:bg-blue-50"><img v-if="team.logo" :src="team.logo" alt="" class="h-8 w-8 object-contain" /><span>{{ team.name }}</span></button></li>
    </ul>
