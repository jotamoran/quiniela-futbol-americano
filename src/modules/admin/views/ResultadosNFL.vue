  ...
  <main class="page-shell max-w-5xl">
    ...
    <form v-if="juegos.length" @submit.prevent="guardar" class="space-y-3">
       <article v-for="juego in juegos" :key="juego.id" class="rounded-2xl border bg-white p-4 shadow-sm" :class="juego.estado === 'cancelado' ? 'border-gray-300 bg-gray-50' : juego.underdog ? 'border-red-200' : 'border-transparent'">
        <div class="grid gap-3 sm:grid-cols-[1fr_7rem_7rem_10rem] sm:items-end"><div class="min-w-0"><div class="flex min-w-0 items-center gap-3"><img v-if="juego.logo_visitante" :src="juego.logo_visitante" alt="" loading="lazy" decoding="async" class="h-9 w-9 shrink-0 object-contain" /><strong class="min-w-0 break-words">{{ juego.equipo_visitante }} @ {{ juego.equipo_local }}</strong><img v-if="juego.logo_local" :src="juego.logo_local" alt="" loading="lazy" decoding="async" class="h-9 w-9 shrink-0 object-contain" /></div><div class="mt-2 flex flex-wrap gap-1"><span v-if="juego.desempate" class="rounded-full bg-blue-50 px-2 py-1 text-xs font-bold text-quiniela-azul">Desempate</span><span v-if="juego.underdog" class="rounded-full bg-red-50 px-2 py-1 text-xs font-bold text-quiniela-rojo">Underdog</span><span v-if="juego.estado === 'cancelado'" class="rounded-full bg-gray-200 px-2 py-1 text-xs font-bold text-gray-600">Cancelado</span><span v-else-if="esEmpate(juego)" class="rounded-full bg-amber-50 px-2 py-1 text-xs font-bold text-amber-800">Empate</span></div></div>
        ...
        <div class="mt-3 flex flex-col gap-2 border-t border-gray-100 pt-3 sm:flex-row"><button type="button" @click="cambiarJuego(juego, 'underdog')" :disabled="semanaFinalizada || configurando || juego.estado === 'cancelado'" class="min-h-11 rounded-xl border border-quiniela-rojo px-3 py-2 text-sm font-semibold text-quiniela-rojo disabled:opacity-40">{{ juego.underdog ? 'Underdog actual' : 'Marcar como underdog' }}</button><button type="button" @click="cambiarJuego(juego, 'cancelado')" :disabled="semanaFinalizada || Boolean(configurando)" class="min-h-11 rounded-xl border border-gray-300 px-3 py-2 text-sm font-semibold text-gray-700 disabled:opacity-40">{{ juego.estado === 'cancelado' ? 'Reactivar partido' : 'Cancelar partido' }}</button></div>
      </article>
      <section class="sticky bottom-3 z-20 rounded-2xl border border-gray-200 bg-white/95 p-3 shadow-xl backdrop-blur sm:flex sm:items-center sm:justify-between sm:gap-4 sm:p-4"><div><p class="text-sm font-semibold text-quiniela-azulOscuro">{{ juegos.filter((juego) => juego.estado === 'finalizado').length }} partidos finalizados</p><p v-if="erroresResultados.length" role="alert" class="mt-1 text-xs text-amber-700">{{ erroresResultados[0] }}</p><p v-else class="mt-1 text-xs text-gray-500">{{ semanaFinalizada ? 'Semana finalizada: información de solo consulta.' : 'Verifica marcadores y estados antes de guardar.' }}</p></div><button :disabled="semanaFinalizada || guardando || erroresResultados.length > 0" class="min-h-11 w-full rounded-xl bg-quiniela-azul px-5 py-3 font-bold text-white disabled:opacity-50 sm:w-auto">{{ guardando ? 'Guardando…' : 'Guardar resultados' }}</button></section>
    </form>
