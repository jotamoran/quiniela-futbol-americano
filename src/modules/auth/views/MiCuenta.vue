...
import { actualizarDatosBancariosNFL, obtenerDatosBancariosNFL } from '@/services/nflService';
...
async function onGuardarDatosBancarios() {
  ...
  try {
    await actualizarDatosBancariosNFL({ banco: banco.value.trim(), clabe: clabe.value.trim(), titular: titular.value.trim() });
...
onMounted(async () => {
  ...
  try {
    const datos = await obtenerDatosBancariosNFL();
...
<template>
  <main class="page-shell max-w-2xl">
    ...
    <section class="flex items-center gap-4 rounded-2xl bg-quiniela-azulOscuro p-4 text-white shadow-sm sm:p-5">
      <img src="@assets/logo.webp" alt="Quiniela Futbol Americano" class="h-auto w-36 shrink-0 object-contain sm:w-44" />
    ...
    <form @submit.prevent="onGuardarNombre" class="space-y-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
      <h2 class="font-bold text-quiniela-azulOscuro">Nombre completo</h2>
      ...
      <button type="submit" :disabled="guardandoNombre" class="min-h-11 w-full rounded-xl bg-quiniela-azul px-5 py-2.5 font-semibold text-white disabled:opacity-50 sm:w-auto">{{ guardandoNombre ? 'Guardando…' : 'Guardar nombre' }}</button>
    </form>

    <form @submit.prevent="onGuardarCorreo" class="space-y-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
      <h2 class="font-bold text-quiniela-azulOscuro">Correo</h2>
      ...
      <button type="submit" :disabled="guardandoCorreo" class="min-h-11 w-full rounded-xl bg-quiniela-azul px-5 py-2.5 font-semibold text-white disabled:opacity-50 sm:w-auto">{{ guardandoCorreo ? 'Enviando…' : 'Cambiar correo' }}</button>
    </form>

    <form @submit.prevent="onGuardarPassword" class="space-y-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
      <h2 class="font-bold text-quiniela-azulOscuro">Contraseña</h2>
      ...
      <button type="submit" :disabled="guardandoPassword" class="min-h-11 w-full rounded-xl bg-quiniela-azul px-5 py-2.5 font-semibold text-white disabled:opacity-50 sm:w-auto">{{ guardandoPassword ? 'Guardando…' : 'Cambiar contraseña' }}</button>
    </form>
    ...
    <form v-if="authStore.isAdmin" @submit.prevent="onGuardarDatosBancarios" class="space-y-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
      <h2 class="font-bold text-quiniela-azulOscuro">Datos bancarios</h2>
      ...
      <button type="submit" :disabled="guardandoBanco" class="min-h-11 w-full rounded-xl bg-quiniela-azul px-5 py-2.5 font-semibold text-white disabled:opacity-50 sm:w-auto">{{ guardandoBanco ? 'Guardando…' : 'Guardar datos bancarios' }}</button>
    </form>
  </main>
</template>
