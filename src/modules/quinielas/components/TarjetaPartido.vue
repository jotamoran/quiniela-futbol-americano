...
<template>
  <article class="rounded-2xl border bg-white p-4 shadow-sm transition" :class="props.underdogSeleccionado ? 'border-quiniela-rojo ring-2 ring-red-100' : juego.estado === 'cancelado' ? 'border-gray-200 bg-gray-50 opacity-70' : 'border-gray-200'">
    <div class="mb-3 flex flex-col items-start gap-2 text-xs text-gray-500 sm:flex-row sm:items-center sm:justify-between">
      ...
      <span class="flex w-full flex-wrap justify-start gap-1 sm:w-auto sm:justify-end"><span v-if="props.underdogSeleccionado" class="rounded-full bg-red-50 px-2 py-1 font-bold text-quiniela-rojo">Tu underdog · +2</span><span v-if="juego.underdog_lado" class="max-w-full rounded-full bg-amber-50 px-2 py-1 font-bold text-amber-800">No favorito: {{ juego.underdog_lado === 'L' ? juego.equipo_local : juego.equipo_visitante }}</span><span v-if="juego.desempate" class="rounded-full bg-blue-50 px-2 py-1 font-bold text-quiniela-azul">Desempate</span><span v-if="juego.estado === 'cancelado'" class="rounded-full bg-gray-200 px-2 py-1 font-bold text-gray-600">Cancelado</span></span>
    </div>
